...
"""Modifies ONNX model to be compatible with NVIDIA TensorRT."""
...
def count_uint8(graph):
  count = len([
      ...
      if hasattr(tensor, "dtype") and tensor.dtype == np.uint8
  ])
...
def fix_uint8_tensors(graph):
  ...
  Args:
    graph: The onnx graph to fix.

  Returns:
    The onnx graph with fixed tensors.
  """
  for node in graph.nodes:
    for node_input_tensor in node.inputs:
      if hasattr(node_input_tensor,
                 "dtype") and node_input_tensor.dtype == np.uint8:
        node_input_tensor.dtype = np.int32
    for node_output_tensor in node.outputs:
      if hasattr(node_output_tensor,
                 "dtype") and node_output_tensor.dtype == np.uint8:
        node_output_tensor.dtype = np.int32

  return graph


def replace_combined_nms(graph,
                         top_k=1284,
                         keep_top_k=100,
                         num_classes=3,
                         plugin_version="1"):
  """Replaces combined NMS operation.

  Although, in principle, the value of top_k, keep_top_k, num_classes should be
  able to be inferred from the graph.

  Due to the limitation of the ONNX parser, we currently are not able to get
  these values.

  Args:
    graph: The frozen graph to replace combined NMS.
    top_k: topK argument to pass to new Batched NMS TRT.
    keep_top_k: keepTopK argument to pass to new Batched NMS TRT.
    num_classes: numClasses argument to pass to new Batched NMS TRT.
    plugin_version: pluginVersion argument to pass to new Batched NMS TRT.

  Returns:
    The manipulated onnx graph with CombinedNMS converted to BatchedNMS_TRT.
  """
  ...
  output_classes = gs.Variable(
      name="detection_classes_nms:0",
      dtype=np.float32,
      shape=(batch_size, keep_top_k))
  output_num_detections = gs.Variable(
      name="num_detections_nms:0", dtype=np.int32, shape=(batch_size, 1))
  ...
  for node in graph.nodes:
    if node.name == "Postprocessor/CombinedNonMaxSuppression/combined_non_max_suppression/CombinedNonMaxSuppression":
      ...
      node.outputs = [
          output_num_detections, output_boxes, output_scores, output_classes
      ]
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--onnx", help="onnx model path", default="model_opset_10.onnx")
  parser.add_argument(
      "--modified",
      help="modified onnx model path",
      default="model_opset_10_modified.onnx")
  args = parser.parse_args()
  modify_onnx(
      onnx_model_filepath=args.onnx, modified_onnx_model_filepath=args.modified)
